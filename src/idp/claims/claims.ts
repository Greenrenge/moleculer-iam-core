...
import {
  ...
  IdentityClaimsSchemaPayload,
  IdentityClaimsSchemaPayloadValidationSchema,
} from "./types"
...
export class IdentityClaimsManager {
  ...
  constructor(
    ...
    opts?: Partial<IdentityClaimsManagerOptions>,
  ) {
    this.logger = props.logger || console

    // compile payload validation functions
    // @ts-ignore
    this.validatePayload = validator.compile(
      IdentityClaimsSchemaPayloadValidationSchema,
    )
    ...
    this.options = _.defaultsDeep(
      opts || {},
      defaultIdentityClaimsManagerOptions,
    )
    this.mandatoryScopes = [
      ...new Set(this.options.mandatoryScopes!.concat(["openid"])),
    ]
  }
  ...
  private createClaimsSchema(payload: IdentityClaimsSchemaPayload) {
    ...
    const { code, error } = Terser.minify(
      `(${payload.migration!})(oldClaim, claims);`,
      {
        ecma: 6,
        compress: false,
        mangle: false,
  ...
  private compileClaimsValidator(schema: IdentityClaimsSchema) {
    ...
    return async (claims: { [claimKey: string]: any }): Promise<void> => {
  ...
  private compileClaimsMigrationStrategy(schema: IdentityClaimsSchema) {
    ...
    try {
      ...
      throw new IAMErrors.ValidationFailed([], {
  ...
  private async syncSupportedScopes() {
    ...
    this._supportedScopes = await this.getActiveClaimsSchemata().then(
      (schemata) =>
  ...
  public async forceReloadClaims(args: {
    ...
    try {
      ...
      if (args.where) {
        ...
        let offset = 0
        while (true) {
          const ids = await this.adapter.get({
            where: args.where,
            offset,
            limit,
          })
  ...
  public async defineClaimsSchema(
    payload: IdentityClaimsSchemaPayload,
    ...
    try {
      ...
      const scopeFilter: WhereAttributeHash = {
        metadata: { scope: { [schema.scope]: true } },
      }

      // compile claims schema and validate it with default value
      const validateClaims = await this.compileClaimsValidator(schema)
      ...
      const inactiveSchema: IdentityClaimsSchema | void =
        await this.adapter.getClaimsSchema({
          key: schema.key,
      ...
      if (inactiveSchema) {
        this.logger.info(
          `activate identity claims schema for ${
            schema.key
          }:${schema.version.substr(0, 8)}`,
        )
        // tslint:disable-next-line:no-shadowed-variable
        const transaction = await this.adapter.transaction()
        try {
          // activate
          await this.adapter.setActiveClaimsSchema(
            { key: schema.key, version: schema.version },
            transaction,
          )
          ...
          let offset = 0
          while (true) {
            const identities = await this.adapter.get({
              offset,
              limit,
              where: scopeFilter,
            })
            ...
            if (this.adapter.touchMigrationLock) {
              await this.adapter.touchMigrationLock(
                schema.key,
      ...
      const activeSchema: IdentityClaimsSchema | void =
        await this.adapter.getClaimsSchema({ key: schema.key, active: true })

      // if has exactly same schema
      if (activeSchema && activeSchema.version === schema.version) {
        this.logger.info(
          `skip identity claims schema migration for ${
            activeSchema.key
          }:${activeSchema.version.substr(0, 8)}`,
        )
      ...
      if (schema.parentVersion) {
        // from specific version
        parentSchema = await this.adapter.getClaimsSchema({
          key: schema.key,
          version: schema.parentVersion,
        })
        if (!parentSchema) {
          throw new IAMErrors.ValidationFailed([], {
      ...
      this.logger.info(
        `start identity claims migration: ${schema.key}:${
          schema.parentVersion ? schema.parentVersion.substr(0, 8) + " -> " : ""
        }${schema.version.substr(0, 8)}`,
      )
      // begin transaction
      const transaction = await this.adapter.transaction()
      try {
          ...
          await Promise.all(
            ids.map(async (id, index) => {
              ...
              try {
                ...
                await this.adapter.createOrUpdateVersionedClaims(
                  id,
                  [
                    {
                      key: schema.key,
                ...
                if (JSON.stringify(oldClaim) !== JSON.stringify(newClaim)) {
                  await this.adapter.onClaimsUpdated(
                    id,
                    { [schema.key]: newClaim },
                    transaction,
                  )
        ...
        this.logger.info(
          `identity claims migration finished: ${schema.key}:${
            schema.parentVersion
              ? schema.parentVersion.substr(0, 8) + " -> "
              : ""
          }${schema.version.substr(0, 8)}`,
        )
        return schema
      } catch (error) {
        // failed to migrate, revoke migration
