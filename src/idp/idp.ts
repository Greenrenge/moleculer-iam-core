...
import {
  IDPAdapter,
  IDPAdapterConstructors,
  IDPAdapterConstructorOptions,
  Transaction,
} from "./adapter"
...
export class IdentityProvider {
  ...
  constructor(
    ...
    opts?: Partial<IdentityProviderOptions>,
  ) {
    ...
    if (options.adapter instanceof IDPAdapter) {
      ...
      const adapterKey: keyof typeof IDPAdapterConstructors =
        Object.keys(IDPAdapterConstructors).find(
          (k) =>
            k.toLowerCase() ===
            ((options.adapter as any)!.type || "").toLowerCase(),
  ...
  public async find(args: WhereAttributeHash): Promise<Identity | undefined> {
    ...
    if (
      !where.metadata ||
      typeof where.metadata === "undefined" ||
      Object.keys(where.metadata).length === 0
    ) {
    ...
    return this.adapter
      .find(where)
      .then((id) => (id ? new Identity({ id, provider: this }) : undefined))
  }
  ...
  //TODO: green create account that save email password
  /* create account */
  public async create(
    args: {
    ...
    return this.adapter
      .create(args as any, transaction, ignoreUndefinedClaims)
