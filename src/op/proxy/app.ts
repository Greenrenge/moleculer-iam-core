...
export class ProviderApplicationBuilder {
  ...
  public _dangerouslySetPrefix(prefix: string) {
    ...
    this.logger.info(
      `OIDC application route path configured:`,
      `${prefix}/:path`,
    )
  }
  ...
  public readonly getURL = (path: string, withHost?: true) =>
    `${withHost ? this.builder.issuer : ""}${this.prefix}${path}`

  private readonly wrapContext: IMiddleware<any, ApplicationRequestContext> =
    async (ctx, next) => {
      ctx.idp = this.idp
      ctx.op = await new OIDCProviderContextProxy(
        ctx,
        this.builder,
      )._dangerouslyCreate()
      ctx.unwrap = () => {
  ...
  private readonly errorHandler: IMiddleware<any, ApplicationRequestContext> =
    async (ctx, next) => {
      try {
        await next()
      } catch (err: any) {
        ...
        let normalizedStatus =
          status || statusCode || code || status_code || 500
        ...
        const normalizedError = {
          ...
          error_description:
            error_description || message || "Unexpected error.",
  ...
  public setRendererFactory<F extends ApplicationRendererFactory>(
    factory: F,
    options?: F extends ApplicationRendererFactory<infer O> ? O : never,
  ) {
  ...
  public getRoutes(promptName?: string): ApplicationRoutes {
    if (!this._routesFactory) {
      this.logger.warn(
        "routes factory not configured; which is to ensure available xhr/page request endpoints for each prompts",
      )
      return {}
    }
  ...
  private readonly renderErrorProxy: NonNullable<
    DynamicConfiguration["renderError"]
  > = (ctx, out, error) => {
  ...
  private renderLogout: (
    ctx: ApplicationRequestContext,
    xsrf: string,
  ) => Promise<void> = async (ctx, xsrf) => {
  ...
  private readonly logoutSourceProxy: NonNullable<
    DynamicConfiguration["logoutSource"]
  > = (ctx) => {
    // @ts-ignore
    return this.wrapContext(ctx as any, () => {
  ...
  private renderLogoutEnd: (ctx: ApplicationRequestContext) => Promise<void> =
    async (ctx) => {
  ...
  private readonly postLogoutSuccessSourceProxy: NonNullable<
    DynamicConfiguration["postLogoutSuccessSource"]
  > = async (ctx) => {
    return this.wrapContext(ctx as any, async () => {
      const op: ApplicationRequestContext["op"] = ctx.op as any
      op.clientMetadata = await op.getPublicClientProps(
        ctx.oidc.client as Client,
      )
  ...
  private renderDeviceFlow: (
    ...
    userCode: string,
    xsrf: string,
  ) => Promise<void> = async (ctx, userCode, xsrf) => {
  ...
  private readonly deviceFlowUserCodeInputSourceProxy: NonNullable<
    DeviceFlowConfiguration["userCodeInputSource"]
  > = (ctx, formHTML, out, error) => {
    return this.wrapContext(ctx as any, () => {
      const op: ApplicationRequestContext["op"] = ctx.op as any
      // @ts-ignore
      ...
      const xsrf = op.session.state && op.session.state.secret
      return this.renderDeviceFlow(
        ctx as any,
        ctx.oidc.params!.user_code || "",
        xsrf,
      )
    })
  }
  ...
  private renderDeviceFlowConfirm: (
    ...
    xsrf: string,
    device: DeviceInfo,
  ) => Promise<void> = async (ctx, userCode, xsrf, device) => {
  ...
  private readonly deviceFlowUserCodeConfirmSourceProxy: NonNullable<
    DeviceFlowConfiguration["userCodeConfirmSource"]
  > = (ctx, formHTML, client, device, userCode) => {
    return this.wrapContext(ctx as any, () => {
      const op: ApplicationRequestContext["op"] = ctx.op as any
      // @ts-ignore
      ctx.assert(op.user && op.client)
  ...
  private readonly deviceFlowSuccessSourceProxy: NonNullable<
    DeviceFlowConfiguration["successSource"]
  > = (ctx) => {
    return this.wrapContext(ctx as any, () => {
  ...
  public _dangerouslyBuild() {
    ...
    this.op.app.use(
      compose<any>([
        ...
        //TODO: green bind the renderer routes to the app
