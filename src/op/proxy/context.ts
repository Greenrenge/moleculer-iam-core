...
import {
  ...
  ApplicationState,
  ApplicationResponse,
  ApplicationSessionPublicState,
  ApplicationMetadata,
  ApplicationRoutes,
  ApplicationSessionSecretState,
} from "./app.types"
import {
  Client,
  DeviceInfo,
...
export class OIDCProviderContextProxy {
  ...
  constructor(
    private readonly ctx: ApplicationRequestContext,
    private builder: ProviderConfigBuilder,
  ) {}
  ...
  public readonly getNamedURL = (name: string, opts?: any) => {
    return (this.provider as any)
      .urlFor(name, opts)
      .replace(this.builder.issuer, "")
  }

  public get routes() {
    return this.builder.app.getRoutes(
      this.interaction &&
        this.interaction.prompt &&
        this.interaction.prompt.name,
    )
  }
  ...
  public async render(
    name: string,
    error?: OIDCError,
    additionalRoutes?: ApplicationRoutes,
  ): Promise<void> {
    ...
    //TODO: green render renderer, put ctx.body  to have html (header,  state, footer)
  ...
  public async redirectWithUpdate(
    promptUpdate:
      | Partial<InteractionResults>
  ...
  public setSessionPublicState(
    update: (
      prevPublicState: ApplicationSessionPublicState,
    ) => ApplicationSessionPublicState,
  ): void {
  ...
  public setSessionSecretState(
    update: (
      prevSecretState: ApplicationSessionSecretState,
    ) => ApplicationSessionSecretState,
  ): void {
    return this.setSessionState((prevState) => ({
      ...prevState,
  ...
  public async getPublicClientProps(
    client?: Client,
  ): Promise<Partial<ClientMetadata> | undefined> {
    if (!client) return
    return {
  ...
  public async getPublicUserProps(
    id?: Identity,
  ): Promise<Partial<OIDCAccountClaims> | undefined> {
    if (!id) return
    const { sub, email, picture, name } = await id.claims(
      "userinfo",
      "profile email",
    )
    return {
  ...
  public async getAuthorizedClientsProps() {
    ...
    return Promise.all(
      Object.keys(authorizations).map(async (clientId) =>
        this.provider.Client.find(clientId)
          ...
          .then((clientProps) => {
            ...
            return clientProps as Partial<ClientMetadata> & {
  ...
  public async _dangerouslyCreate() {
    ...
    if (!ctx.oidc) {
      Object.defineProperty(ctx, "oidc", {
        value: new hiddenProvider.OIDCContext(ctx as any),
      })
    }

    const originalSessionState =
      // @ts-ignore ensure session
      (ctx.oidc.session && ctx.oidc.session.state) || {}
    // @ts-ignore ensure session
  ...
  private async readProviderSession() {
    ...
    this.user = this.session.account
      ? await idp.findOrFail({ id: this.session.account })
      : undefined
    if (this.user) {
    ...
    try {
      ...
      this.client = interaction.params.client_id
        ? ((await provider.Client.find(interaction.params.client_id)) as Client)
        : undefined
      if (this.client) {
