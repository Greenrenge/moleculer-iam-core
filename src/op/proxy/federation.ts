...
export type IdentityFederationProviderConfiguration<
  CustomProfile extends {},
...
export type IdentityFederationStrategyFactory<
  CustomProfile extends {},
  CustomOptions extends {},
> = (
  options: Omit<CustomOptions, ForbiddenCustomOptions> & {
    clientID: string
    clientSecret?: string
    scope: string
    callbackURL: string
  ...
  verify:
    | IdentityFederationCallbackVerifyFunction<CustomProfile>
    | AppleStrategy.VerifyFunction,
) => Strategy
...
export type IdentityFederationCallbackHandler<CustomProfile extends {}> = (
  args: IdentityFederationCallbackArgs<CustomProfile>,
) => Promise<Identity>
...
export type IdentityFederationCallbackVerifyFunction<CustomProfile extends {}> =
  (
    ...
    profile: CustomProfile,
      ...
      error: Error | null,
      callbackArgs?: IdentityFederationCallbackArgs<CustomProfile>,
      info?: any,
    ) => void,
  ) => void
...
export class IdentityFederationBuilder {
  ...
  private readonly callbacks: {
    [provider: string]: IdentityFederationCallbackHandler<any>
  } = {}
  ...
  public setCallbackPrefix(prefix: string) {
    ...
    this.builder.logger.info(
      `OIDC federation route path configured:`,
      `${this.builder.app.prefix}${prefix}/:path`,
    )
    return this
  }

  public readonly getCallbackURL = (providerName: string) =>
    this.builder.app.getURL(`${this._prefix}/${providerName}`, true)
  ...
  public setProviderConfigurationMap(
    configMap: IdentityFederationProviderConfigurationMap,
  ) {
    this.builder.assertBuilding()
  ...
  public _dangerouslyBuild() {
    ...
    for (const [provider, options] of Object.entries(this.config)) {
        ...
        provider === "apple"
          ? strategy(
              ...
              async (
                ...
                this.builder.logger.info(
                  `try identity federation from ${provider} with ${this.scopes[
                    provider
                  ].join(", ")} scopes:`,
                  profile,
                )
      ...
      this.builder.logger.info(
        `enable identity federation from ${provider} with ${this.scopes[
          provider
        ].join(", ")} scopes: ${callbackURL}`,
      )
    }
  }
      ...
      this.passport
        .authenticate(
          ...
          async (
            err: any | undefined,
            args: Omit<
              IdentityFederationCallbackArgs<any>,
              "scope" | "idp" | "logger"
            >,
          ) => {
            try {
