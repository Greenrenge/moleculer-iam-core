...
export function buildLoginRoutes(
  ...
  builder.app.router // redirect to initial render page
    ...
    .get("/login", async (ctx, next) => {
      ...
      if (user) {
        const changeAccount =
          ctx.query.change_account === "true" ||
          interaction!.params.change_account === "true"
        ...
        if (
          ...
          return ctx.op.redirect(
            `/login?email=${encodeURIComponent(userClaims.email)}`,
          )
        }
      }
    ...
    .post("/login/check_email", async (ctx) => {
      const user = await ctx.idp.findOrFail({
        //@ts-ignore
        claims: { email: ctx.request.body.email || "" },
      })
    ...
    .post("/login/check_password", async (ctx) => {
      ...
      const verified = await user.assertCredentials({
        password: password || "",
      })
      if (verified === null) {
