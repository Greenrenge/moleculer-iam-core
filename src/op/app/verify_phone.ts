...
async function defaultSend({ logger, ...args }: IdentityPhoneVerificationArgs) {
  logger.warn(
    "should implement op.app.verifyPhone.send option to send phone verification message",
    args,
  )
}

export function buildVerifyPhoneRoutes(
  ...
  builder.app.router
    ...
    .post("/verify_phone/check_phone", async (ctx) => {
      ...
      // update session
      if (
        !ctx.op.sessionPublicState.verifyPhone ||
        ctx.op.sessionPublicState.verifyPhone.phoneNumber !==
          claims.phone_number ||
    ...
    .post("/verify_phone/send", async (ctx) => {
      ...
      if (publicState && publicState.verifyPhone) {
        ...
        if (
          verifyState.phoneNumber === claims.phone_number &&
          verifyState.expiresAt &&
          moment().isBefore(verifyState.expiresAt)
        ) {
    ...
    .post("/verify_phone/verify", async (ctx) => {
      ...
      if (
        publicState.phoneNumber !== claims.phone_number ||
        moment().isAfter(publicState.expiresAt) ||
