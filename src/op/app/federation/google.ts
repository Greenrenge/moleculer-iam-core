...
import {
  IOAuth2StrategyOption as StrategyOption,
  OAuth2Strategy as Strategy,
  Profile,
} from "passport-google-oauth"
import {
  IdentityFederationProviderConfiguration,
  OIDCAccountClaims,
} from "../../proxy"
import { OIDCProviderProxyErrors } from "../../proxy/error"

export type GoogleProviderConfiguration =
  IdentityFederationProviderConfiguration<Profile, StrategyOption>
...
export const googleProviderConfiguration: GoogleProviderConfiguration = {
  ...
  callback: async (props) => {
    ...
    const metadata = {
      federation: { google: { id: claims.sub, hd: claims.hd || null } },
    }
    //@ts-ignore
    delete claims.sub
    delete claims.hd
    ...
    try {
      ...
      if (
        ...
        response.phoneNumbers[0] &&
        response.phoneNumbers[0].value
      ) {
        ...
        if (result === true) {
          ...
          props.logger.error(
            "failed to validate phone_number from google",
            result,
          )
        }
      }
      ...
      if (response.error && response.error.error) {
        props.logger.error(
          "failed to fetch phone_number from google",
          //@ts-ignore
