...
async function defaultSend({ logger, ...args }: IdentityEmailVerificationArgs) {
  logger.warn(
    "should implement op.app.verifyEmail.send option to send email verification mail",
    args,
  )
}

export function buildVerifyEmailRoutes(
  ...
  builder.app.router
    ...
    .post("/verify_email/check_email", async (ctx) => {
      ...
      // update session
      if (
        !ctx.op.sessionPublicState.verifyEmail ||
    ...
    .post("/verify_email/send", async (ctx) => {
      ...
      if (publicState && publicState.verifyEmail) {
        ...
        if (
          verifyState.email === claims.email &&
    ...
    .post("/verify_email/verify", async (ctx) => {
      ...
      if (
        publicState.email !== claims.email ||
